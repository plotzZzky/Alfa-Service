...
class RegisterCustomerTest(TestCase):
    ...
    def test_register_status(self):
        response = self.client.post('/users/register/customer/', json.dumps(self.credentials),
                                    content_type="application/json")
        self.assertEqual(response.status_code, 200)

    def test_register_check_token(self):
        response = self.client.post('/users/register/customer/', json.dumps(self.credentials), content_type="application/json")
        user = User.objects.get(username=self.credentials['username'])
        token = Token.objects.get(user=user)  # type: ignore
        result = json.loads(response.content)
        r = True if "token" in result else False
